...
rdd10 = rdd01.flatMap(lambda x: range(0, x)
...
meteo.groupByKey().reduceByKey( (total,valeur) => total+valeur).take(10).foreach(println)
...
villes.join(meteo).take(5).foreach(println)
